...
class RobotState:
    ...
    def update_output(self):
        ...
        for i in range(3):#5-7
            if  self.gait_changed_num>=2:
                quick_stop_ros.append_data(self.root_pos_target[i]-self.root_pos_des_abs[i]) #self.root_pos_target[0:2] =root_pos_abs[0:2]+self.root_pos_des_abs[0:2] 
            else:
                quick_stop_ros.append_data(0)
        for i in range(3):#8-10
            if self.gait_changed_num>=2:
                quick_stop_ros.append_data(self.root_pos[i]-self.root_pos_des_abs[i])
            else:
                quick_stop_ros.append_data(0)
            
        for i in range(3):#11-13
            quick_stop_ros.append_data(self.root_lin_vel_target[i])
        for i in range(3):#14-16
    ...
    def update_plan(self):
        ...
        if self.use_lip:
            ...
            # x0_max=0.25
            # if delta_foot_x>=0:
            #     delta_foot_x=np.minimum(x0_max,delta_foot_x)
            # else:
            #     delta_foot_x=np.maximum(-x0_max,delta_foot_x)
            # if delta_foot_y>=0:
            #     delta_foot_y=np.minimum(x0_max,delta_foot_y)
            # else:
            #     delta_foot_y=np.maximum(-x0_max,delta_foot_y)
    ...
    def draw_lines(self):
        ...
        root_est_project_pose.p = gymapi.Vec3(*self.root_pos_des_abs_ros)
    ...
    def _root_control(self):
        ...
        self.root_pos_des_abs_ros=self.rot_mat_z@self.root_pos_des_rel+self.root_pos
        if self.use_lip:
        ...
        if(self.quick_stop_):
            ...
            period=count_per_cycle*sim_params.dt
            if np.any(self.time<=period):
                root_pos_abs=np.zeros(3)
